...
fonts/
  variable/
    ...
    ...
  static/
    SarasaUiPropDigitsCL-TTF-1.0.40/
    SarasaUiPropDigitsCL-TTF-Unhinted-1.0.40/
    SarasaUiPropDigitsSC-TTF-1.0.40/
    SarasaUiPropDigitsSC-TTF-Unhinted-1.0.40/
    ...
reports/
...
- `head.fontRevision` 使用 OpenType 16.16 fixed 可表达的项目数值 `1.0400`，对应本仓库版本 `1.0.40`；`name` table 的 nameID 5 写作 `Version 1.0.40 ...`。
...
- `Sarasa-Ui-VF-PropDigits-{REGION}-TTF-1.0.40.zip`：单个地区的正体和 Italic 可变 TTF。
- `SarasaUiPropDigits{REGION}-TTF-1.0.40.zip`：单个地区的 hinted 静态 TTF。
- `SarasaUiPropDigits{REGION}-TTF-Unhinted-1.0.40.zip`：单个地区的 unhinted 静态 TTF。

另有三套全地区合集：

- `Sarasa-Ui-VF-PropDigits-TTF-1.0.40.zip`：全部地区的可变 TTF。
- `SarasaUiPropDigits-TTF-1.0.40.zip`：全部地区的 hinted 静态 TTF。
- `SarasaUiPropDigits-TTF-Unhinted-1.0.40.zip`：全部地区的 unhinted 静态 TTF。

合计 21 个 ZIP。版本号 `1.0.40` 对应本次使用的 Sarasa Gothic 上游基线。
